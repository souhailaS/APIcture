...
export default async function generateGallery(path) {
  ...
  var test_dir_default = path;
  var basePath_default = "https://souhailas.github.io/test_outputs2023";
  var output_dir_default = ".";
  ...
  async function askUserForValues() {
    ...
    output_dir = await getUserInput("Enter the output_dir value: ");
  ...
  async function run() {
    ...
    var repos = fs.readdirSync(test_dir);
    ...
    if (processed.length > 0) {
      ...
      if (answer.toLowerCase() === "y" || answer.toLowerCase() === "yes") {
        ...
        // delete index.html
        if (fs.existsSync(join(output_dir, "test_outputs", "index.html"))) {
          fs.unlinkSync(join(output_dir, "test_outputs", "index.html"));
        }
      }
    ...
    // repos.sort();
    // console.log(repos);
    var markdown = ""; //fs.readFileSync(join(output_dir, "report.md"), "utf8");
    var nextRepo = async (j) => {
      // get the git remote url from the .git
      var repo = repos[j];

      var repoPath = join(test_dir, repo);
        ...
        var nextFile = async (i) => {
          ...
          if (
            ...
            if (
              ...
              // create index.html inside test_outputs
              if (!fs.existsSync(join(output_dir, "test_outputs"))) {
                fs.mkdirSync(join(output_dir, "test_outputs"), { recursive: true });
              }
              if (!fs.existsSync(join(output_dir, "test_outputs", "index.html"))) {
                fs.writeFileSync(
                  join(output_dir, "test_outputs", "index.html"),
                  `<!DOCTYPE html>
<html lang="en">
<head>
   ...
   <style>
    ...
    <body class="container">
      ...
      <hr />
        ...
        <p><a href="${basePath}/test_outputs/viz-${encodeURIComponent(
              ...
              // html+= `
              // <div><iframe src="${basePath}/test_outputs/viz-${encodeURIComponent(oasFiles[i].oaspath.split(".")[0])}-${encodeURIComponent(history_metadata.api_titles[0].title)}-api.html#visualizations" width="100%" height="500px"></iframe></div>
        ...
        <img src="${basePath}/test_outputs/viz-${encodeURIComponent(
                oasFiles[i].oaspath.split(".")[0]
              )}-${encodeURIComponent(
                history_metadata.api_titles[0].title
        ...
        `;
              ...
              fs.writeFileSync(join(output_dir, "test_outputs", "index.html"), page);
            } else {
              console.log(
          ...
          if (i < oasFiles.length) {
            // fs.writeFileSync(join(output_dir, "README.md"), markdown);
            await nextFile(i);
          } else {
